...
The problem occurs because directly invoking an R function from C++ (via `cpp11::function`) creates a "top-level" call that isn't nested in the R call stack.

The fix: instead of calling the function directly, construct a call object and evaluate it *in* the caller's environment using `Rf_eval()`.
